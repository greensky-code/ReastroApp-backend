...
const login = (req, res) => {
  ...
  if (flag) return responses.sendError(res, languageCode, {}, flag[1], flag[0]);
  else if (!req.body)
    return responses.sendError(
      res,
      languageCode,
      {},
      constants.responseMessageCode.CLIENT_ERROR,
      constants.responseMessageCode.CLIENT_ERROR
    );
  else {
    createEditViewProfile.findOne(
      ...
      (error, resData) => {
        if (error) {
          return responses.sendError(
            ...
            constants.responseMessageCode.ERROR_IN_EXECUTION
          );
        } else if (!resData) {
          return responses.sendError(
            res,
            ...
            {},
            "EMAIL NOT_EXISTS",
            constants.responseMessageCode.EMAIL_NOT_EXISTS
          );
        } else {
          if (req.body.password == resData.password) {
            ...
            return responses.sendError(
              ...
              {},
              "INCORRECT PASSWORD",
              constants.responseMessageCode.INCORRECT_PASSWORD
            );
          }
        }
      }
  ...
  createEditViewProfile.findOne({ email: req.body.email }, (error, resData) => {
    if (error) {
      ...
      const mailOptions = {
        ...
        subject: "New Message",
        html:
          "<p>An email has been sent to your registered email id." +
          "http://localhost:4200/" +
          resData._id +
          "</p>",
      };
...
const changepassword = (req, res) => {
  ...
  createEditViewProfile.findOne(query, (error, resData) => {
    if (error) {
      ...
      createEditViewProfile.findOneAndUpdate(
        { _id: resData._id },
        setdata,
        { new: true },
        (err, suc) => {
