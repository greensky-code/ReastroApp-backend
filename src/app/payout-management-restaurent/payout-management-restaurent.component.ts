...
declare var $:any;
@Component({
  selector: 'app-payout-management-restaurent',
  templateUrl: './payout-management-restaurent.component.html',
  styleUrls: ['./payout-management-restaurent.component.css']
})
export class PayoutManagementRestaurentComponent implements OnInit {
  cousins_list: any = [];
  limit: any = 10;
  page = 1;
  tabView: any = 'restaurent';
  ...
  viewTab(tab) {
    this.tabView = tab
    if(this.tabView == 'driverpayout'){
        this.router.navigate(['/payout-management'])
    }
    else{
      this.router.navigate(['/payout-management-restaurent'])
