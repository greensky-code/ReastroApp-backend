...
export class ManageStaffComponent implements OnInit {
  ...
  manadeDetails(page){
    this.page=page
    this.spinner.show()
    this.service.getApi(`api/staff?page=${this.page}&is_active=${this.user.kycRole}&role=${this.staffRoles}`,1).subscribe(res=>{
     if(res.status==200){
      this.spinner.hide()
...
submitdata(){
  this.disables=false
  this.spinner.show()
  if(this.staffRoles && this.user.kycRole && this.driver.toDate && this.driver.fromDate && this.searchName){
  ...
  this.url=`api/staff?&search=${this.searchName}&is_active=${this.user.kycRole}&role=${this.staffRoles}&created_at_after=${this.driver.fromDate}&created_at_before=${this.driver.toDate}`
   ...
   if(this.driver.fromDate && this.driver.toDate){
     this.url=`api/staff?created_at_after=`+this.driver.fromDate+'&created_at_before='+this.driver.toDate
   }
   if(this.driver.fromDate=='' && this.driver.toDate){
    this.spinner.hide()
...
paginationSearch(){
  this.service.getApi(`api/staff?search=${this.searchName}&page=${this.page}` ||`api/staff?is_active=${this.user.kycRole}&page=${this.page}` || `api/staff?role=${this.staffRoles}&page=${this.page}`,1).subscribe(res=>{
    if(res.status==200){
      this.manageStaff=res.body.results
      this.total=res.body.count
    }
...
deleteFunctions(){
  ...
  this.service.delete('api/staff/',this.delete_id,1).subscribe(res=>{
    if(res.status==204){
      this.deletedata=res
